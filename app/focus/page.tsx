...
import { generateFocusQueue, minutesToCards, type FocusFilter } from '@/lib/focus';
import { updateSRS, updateCardState, MAX_SESSION_RETRY, type CardWithState } from '@/lib/srs';
...
export default function FocusPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (minutesParam) {
        ...
        setLimit(minutesToCards(minutes));
      } else if (limitParam) {
  ...
  const loadQueue = async () => {
    ...
    try {
      const initialQueue = await generateFocusQueue(filter, limit);
      setQueue(initialQueue);
      setCurrentIndex(0);
      setSide('front');
      setSessionRetryCount({});
      
      if (initialQueue.length >= limit) {
  ...
  const handleGrade = async (grade: Grade) => {
    ...
    if (grade === 'AGAIN') {
      ...
      if (retryCount < MAX_SESSION_RETRY) {
        // キュー末尾に再追加（ただし上限を超えないように）
        const updatedState = { ...state, ...updates } as UserCardState;
        setQueue(prev => {
          if (prev.length < limit) {
            return [...prev, { card, state: updatedState }];
          }
          return prev;
        });
    ...
    if (nextIndex < queue.length) {
      ...
      // 復習完了
      if (reachedLimit) {
        alert('今日はここまでです。残りは翌日に回します。');
      } else {
        alert('Focus Review完了！');
      }
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen p-4">
        <h1 className="text-2xl font-bold mb-6">Focus Review</h1>
        <p>読み込み中...</p>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen p-4">
        ...
        <div className="space-y-4">
          <p className="text-gray-600 mb-4">Focus Reviewするカードがありません</p>
          
          <div className="bg-white rounded-lg shadow p-4">
            <h2 className="text-lg font-semibold mb-2">フィルタ</h2>
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded ${filter === 'HARD' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
              >
                HARDのみ
              </button>
              <button
                onClick={() => setFilter('AGAIN')}
                className={`px-4 py-2 rounded ${filter === 'AGAIN' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
              >
                AGAINのみ
              </button>
            </div>
  ...
  const isLimitReached = currentIndex + 1 >= limit;
  ...
  return (
    <main className="min-h-screen p-4 max-w-2xl mx-auto">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Focus Review</h1>
        <div className="text-sm text-gray-600">
          進捗: {progress}
          {isLimitReached && <span className="text-red-600 ml-2">（上限到達）</span>}
        </div>
      </div>
      ...
      <div className="mb-4 bg-white rounded-lg shadow p-3">
        <div className="flex gap-2">
          ...
          <button
            onClick={() => setFilter('HARD')}
            className={`px-3 py-1 text-sm rounded ${filter === 'HARD' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            HARDのみ
          </button>
          <button
            onClick={() => setFilter('AGAIN')}
            className={`px-3 py-1 text-sm rounded ${filter === 'AGAIN' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            AGAINのみ
          </button>
        </div>
      ...
      {isLimitReached && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-sm text-yellow-800">
            今日の上限（{limit}枚）に到達しました。残りは翌日に回します。
          </p>
        </div>
      )}
